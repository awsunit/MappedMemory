...
struct proc {
    ...
    List children;                      // list of children that belong to this process
    Node proc_node;                     // used by our parent to keep track of us
    Node child_node;
    struct file *files[PROC_MAX_FILE];  // opened file descriptors
    struct condvar wait_cv;               // cv to update proc struct
    int exit_status;                    // set by call to proc_exit
};
