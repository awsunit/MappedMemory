...
main()
{
    ...
    // Custom test
    // test open with invalid path (directory issue && !FS_CREATE)
    if ((fd = open("/REAMDME/ok.txt", FS_RDONLY, EMPTY_MODE)) != ERR_NOTEXIST) {  // !ERR_FTYPE here
        if (fd > 0) {
            error("opened a file with invalid pathname");
        } else {
            error("[custom] returned wrong error code for invalid path, return value was %d", fd);
        }
    }
